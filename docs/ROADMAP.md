...
- ✅ **Desktop window** — `output/window` `window.Show` opens a `gogpu` GPU window and presents the figure zero-copy via `ggcanvas`, reusing the `Controller`/`State` policy (OUTPUT-SPEC Phase 5; `//go:build !js`; runtime-verified on a desktop GPU — HiDPI rotated-text fixed in the `gogpugg` fork)
- ✅ **GPU scene graph rendering** — `canvas.SceneCanvas` adapter implements `canvas.Canvas` via `scene.Scene`, recording all paths/fills/strokes/text as GPU scene commands. `output/window` now renders directly into the GPU scene graph (no CPU rasterization); supports dash patterns via path flattening and segment decomposition; GPU-first with CPU fallback for headless/testing
