...
- **In progress (post-v0.0.9) — the Output Layer.** [OUTPUT-SPEC.md](OUTPUT-SPEC.md) §11 phasing. **Landed (unreleased):** Phase 1 `GGCanvas → RasterCanvas` rename; Phase 2 `output` core (`Figure`/`Source`/`Sizer`/`Surface`/`LiveSurface`/`Imager`/`Event`/`Render` + blank-import registry, `Plot.Build` returns `output.Figure`); Phase 3 `output/file` + `output/image` surfaces with `file.Save`/`file.Encode` and `image.Render` as the primary output API; Phase 4 `Session`/`Controller` interaction loop (fast-path viewport redraw, slow-path rebuild), tested headless; Phase 5 `output/window` (`window.Show` via `gogpu` + zero-copy `ggcanvas`, reusing the Phase-4 controller), runtime-verified on a desktop GPU; Phase 6 `output/web` (`web.Mount` via CPU raster + Canvas2D `putImageData` or SVG `innerHTML`, reusing the Phase-4 `Session`/`Controller`, with DOM event integration and embedded dev server).
...
- ✅ **Output layer — unified `Surface` model** (OUTPUT-SPEC.md Phases 1–3): `output` core (`Figure`/`Source`/`Sizer`/`Surface`/`LiveSurface`/`Imager`/`Render` + blank-import registry), `output/file` (PNG/SVG/PDF) and `output/image` (in-memory) surfaces; `file.Save`/`file.Encode` (`output/file`) and `image.Render` (`output/image`) are the primary output API. `Plot` has no output methods — all rendering goes through the surface packages. `Plot.Build` returns `output.Figure`.
- ✅ **`io.Writer` outputs** — `file.Encode(ctx, src, w, format, …)` for streaming to HTTP, gzip, blob stores; backed by the file surface's `WithWriter`
- ✅ **Interaction loop** — `output.Session`/`Controller`/`State`/`Action` with fast-path viewport redraw and slow-path rebuild (OUTPUT-SPEC Phase 4); default pan/wheel-zoom controller; async/debounced rebuild via `WithRebuildDelay`/`WithRebuildError`; tested headless with a scripted `LiveSurface`; runnable headless example in `examples/session/`
...
- ✅ **Browser surface** — `output/web` (`//go:build js && wasm`, OUTPUT-SPEC Phase 6): `web.Mount(ctx, src, "container-id")` creates a `<canvas>` in the browser, renders via CPU rasterizer + Canvas2D `putImageData` (default) or SVG `innerHTML` (`WithSVG()`), with DOM event integration (pointer, wheel, resize, double-click) driving `output.Session`/`Controller`. Includes embedded HTML host and `web.Serve` dev server.
