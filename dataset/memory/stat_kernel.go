...
func (e *Engine) LinearFitSE(xCol, yCol dataset.AnyColumn, nOut int) (dataset.Table, error) {
	...
	tCrit := 1.96 //nolint:mnd // z ≈ 1.96 for 95% CI (large-sample normal approximation).
	...
	for i := range nOut {
		...
		// SE of mean response = sqrt(MSE * (1/n + (x-xbar)²/Sxx))
		dx := xi - xbar
...
// Uses local residual variance to estimate SE at each grid point.
func (e *Engine) LoessFitSE(ctx context.Context, xCol, yCol dataset.AnyColumn, nOut int) (dataset.Table, error) {
	...
	k := min(max(int(math.Ceil(alpha*float64(n))), 3), n) //nolint:mnd // Minimum window of 3 data points.
