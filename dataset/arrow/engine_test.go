...
func TestColumnFactory(t *testing.T) {
	...
	ds, err := eng.FromColumns(schema,
		...
		eng.NewInt64Column("id", []int64{10, 20, 30}),
	)
	if err != nil {
		t.Fatal(err)
	}
...
func TestAggregator(t *testing.T) {
	...
	scol := eng.NewStringColumn("name", []string{"charlie", "alice", "bob"})

	// Sum float64
	result, err := eng.Sum(fcol)
	if err != nil {
		t.Fatal(err)
	}
	...
	// MinMax string
	minResult, maxResult, err := eng.MinMax(scol)
	if err != nil {
		t.Fatal(err)
	}
