...
// FilterSQL pushes logical string dialect matching conditions executing constraints zero-copy onto the target backend.
func FilterSQL(ds dataset.Dataset, sqlPredicate string) (dataset.Dataset, error) {
	...
	return nil, fmt.Errorf("SQL Pushdown: target Dataset relies outside backend mapping limits (requires RemoteTable)")
}

// GroupBySQL injects exact aggregation targets onto remote servers executing.
