...
func TestSQLPushdown_LenAndColumns(t *testing.T) {
	...
	// Verify Fast-path COUNT query exactly avoiding Materialize!
	mock.ExpectQuery(`^SELECT COUNT\(\*\) FROM flight_metrics$`).
		WillReturnRows(sqlmock.NewRows([]string{"COUNT(*)"}).AddRow(1420))

	if l := rt.Len(); l != 1420 {
		t.Errorf("expected fast-path SELECT COUNT(*) to return 1420, got %d", l)
	}
}
...
func TestSQLPushdown_Filters(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("Failed attaching mock : %v", err)
	}
	defer db.Close()
	...
	if err != nil {
		t.Fatalf("Pushdown bounds generated err: %v", err)
	}
...
func TestSQLPushdown_FallbackMaterialization(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("Failed mocked setup : %v", err)
	}
	defer db.Close()
	...
	if err != nil {
		t.Fatalf("Expected parsed bounds yielding arrow targets, fell exactly short: %v", err)
	}

	if col.Len() != 3 {
		t.Errorf("Expected mocked fallback buffering 3 elements, got %d", col.Len())
	}

	// Ensure mocked tracking resolved zero-copy execution constraints!
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Errorf("Mock bounds not satisfied : %v", err)
	}
}
